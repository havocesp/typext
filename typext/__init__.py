...
"""
    ...
    ===========================================================================
    Description: builtin extended version
    ===========================================================================
"""
import sys
...
import requests
from term import (
    red, green, bold, dim, reverse, white, blue, black, yellow, blink, hide, off, magenta, cyan, bgcyan, underscore, bgwhite,
    bgyellow, bgred, bgmagenta, bggreen, bgblue, bgblack, format as fmt
)
from forbiddenfruit import curse as improve

from utils import cnum, _redneg
...
def _better_formatting():
    ...
    improve(str, "fmt", lambda self, fmt_ = '{}': fmt_.format(float(self)) if fmt_[:-2] in ['f}', 'd}'] else fmt_.format(self))
    improve(int, "fmt", lambda self, fmt_ = '{:d}': fmt_.format(self))
    # improve(float, "cstr", lambda self, ndecims=5, signed_pos=False: \
    # ('\{:+.{:d}f\}'.format(ndecims)).format(self) if signed_pos else )

    for t in _FMT_LIKE_NUM:

        improve(t, "fail2color", lambda self, cond, tcolor = green, fcolor = red: fmt(self, tcolor) if cond else fmt(self, fcolor))
        improve(t, "fraction", lambda self, other: float("{}.{}".format(self, other)))
        improve(t, "redneg", lambda self, fmt_ = '{.f}': _redneg(float(self), fmt_ = fmt_))
...
DEFAULT_FEATURES = ["common", "better_formatting", "functional_iterables",
                    "numbers"]
